...
# 0 = minimal
# 1 = events only
# 2 = events + telemetry (1 Hz)

# =========================================================
# --- LOGGING
# =========================================================

def log_event(time, mode, altitude, velocity, throttle, fuel_ratio, parachute):
    if LOG_LEVEL < 2:
        return
    ...
    print(
        f"[t={time:5.1f}] "
        f"{mode:9} | "
        f"alt={altitude:6.0f}m | "
        f"vel={velocity:6.1f} | "
        f"thr={throttle:4.2f} | "
        f"fuel={fuel_ratio:4.2f}"
    )

def log_event_simple(time, msg):
    if LOG_LEVEL >= 1:
        print(f"[t={time:5.1f}] EVENT: {msg}")
...
def run_sim(vehicle, label):
    ...
    while True:
        ...
        if mode != last_mode:
            ...
            if LOG_LEVEL >= 1:
                print(f"[t={time:5.1f}] {last_mode} → {mode}")

            # landing event
            if mode == "landing":
                log_event_simple(time, "landing")
        ...
        if parachute == "inactive" and velocity < -5 and altitude < 2000:
            parachute = "semi"
            log_event_simple(time, "chute SEMI")

        if parachute == "semi" and altitude < chute["deploy_altitude"]:
            parachute = "full"
            log_event_simple(time, "chute FULL")
        ...
        # TELEMETRY (1 Hz)
        if LOG_LEVEL >= 2 and abs((time % 1.0)) < DT:
            log_event(time, mode, altitude, velocity, throttle, fuel_ratio, parachute)
...
if __name__ == "__main__":
    ...
    print("\n=== v13.3.2 Unified Flight System (Clean Logging) ===")
