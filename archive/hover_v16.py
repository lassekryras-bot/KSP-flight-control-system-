...
def run_sim(vehicle, label):
    ...
    while True:
        ...
        if mode != last_mode:
            print(f"\n=== MODE CHANGE @ {round(time,2)}s ===")
            print(f"{last_mode} → {mode}")
        ...
        if parachute == "inactive" and velocity < -5 and altitude < 2000:
            parachute = "semi"
            print(f"[EVENT] Parachute semi-deploy @ {round(time,2)}s")

        if parachute == "semi" and altitude < chute["deploy_altitude"]:
            parachute = "full"
            print(f"[EVENT] Parachute FULL deploy @ {round(time,2)}s")
...
if __name__ == "__main__":
    ...
    print("\n=== v13.3 Unified Flight System (Logging Enabled) ===")
